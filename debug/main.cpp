...
int main(int argc, char**argv)
{
   ...
   625);
    ...
    // use smart pointer to manage memory of galif
    boost::shared_ptr<sse::Galif> galif = boost::make_shared<sse::Galif>(params);

    sse::KeyPoints_t keypoints; // vector<vector<float> > --> KeyPoints_t sse::Features_t features; // vector<vector<float> > features
